...
def _btn_value(company: str, context: str, channel: str) -> str:
    return json.dumps({"company": company, "context": context, "channel": channel})


def result_modal(email: str, company: str, context: str, channel: str) -> dict:
    """Modal view showing the drafted email with action buttons."""
    val = _btn_value(company, context, channel)
    meta = json.dumps({"email": email, "company": company, "context": context, "channel": channel})
    return {
        "type": "modal",
...
def handle_modal_submit(ack, body):
    ...
    try:
        ...
        ack(response_action="update", view=result_modal(email, company, context, channel))
    except Exception as e:
...
def handle_regenerate(ack, body, client):
    ...
    try:
        ...
        client.views_update(view_id=view_id, view=result_modal(email, val["company"], val["context"], val["channel"]))
    except Exception as e:
    ...
    try:
        client.views_push(
            ...
            view={
                ...
                "private_metadata": json.dumps({"company": val["company"], "channel": val["channel"]}),
...
# ---------------------------------------------------------------------------
# Handler 7: "Approve Draft" button → post email to Slack channel
# ---------------------------------------------------------------------------

@bolt_app.action("approve_draft")
def handle_approve_draft(ack, body, client):
    ...
    client.chat_postMessage(channel=channel, text=email)
    client.views_update(
        view_id=view_id,
        view={
            "type": "modal",
            ...
            "blocks": [
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": ":white_check_mark: Draft posted to the channel."},
                }
            ],
        },
    )
